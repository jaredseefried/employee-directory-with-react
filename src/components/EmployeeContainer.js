...
class Employees extends Component {
  ...
  sortName = () => {
    ...
    if (!this.state.nameSort) {
      ...
      this.setState({
        ...this.state,
        employeesArr: sortedArr,
        nameSort: "ascending",
      });
    } else if (this.state.nameSort === "ascending") {
      const sortedArr = newEmployeesArr
  ...
  handleInputChange = (event) => {
    ...
    const filter = employees.filter((employee) =>
      Object.values(employee).join(" ").toLowerCase().includes(value.toLowerCase())
    );
    this.setState((s) => ({ ...s, employeesArr: filter }));
  };
